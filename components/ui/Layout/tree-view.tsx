              ...
              <a href="#">
                <AccordionPrimitive.Root type="single" collapsible>
                  <AccordionPrimitive.Item value="item-1">
                    <AccordionTrigger>
                      <FolderPlus className="h-6 w-6 shrink-0 mr-5" aria-hidden="true" />
                      {item.name}
                    </AccordionTrigger>
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn("flex flex-1 items-center py-4 font-medium transition-all first:[&[data-state=open]>svg]:rotate-90", className)}
      {...props}
    >
